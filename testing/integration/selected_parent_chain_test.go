...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of spectred instances
	spectred1, spectred2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := spectred1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In spectred2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, spectred2)
	}

	// Connect the two spectreds. This should trigger sync
	// between the two nodes
	connect(t, spectred1, spectred2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := spectred1.rpcClient.GetVirtualSelectedParentChainFromBlock(
