...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("spectred-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("spectred-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("SPECTRED-SYNCER",
		"spectred",
	...
	syncedCmd, err := common.StartCmd("SPECTRED-SYNCED",
		"spectred",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
