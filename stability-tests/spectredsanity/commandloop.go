...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("spectredsanity-spectred-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("spectred", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "SPECTRED-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "SPECTRED-STDERR")
