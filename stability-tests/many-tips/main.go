...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("spectred-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("spectred datadir: %s", dataDir)

	spectredCmd, err := common.StartCmd("SPECTRED",
		"spectred",
	...
	spawn("startNode-spectredCmd.Wait", func() {
		err := spectredCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("spectredCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes spectred closes ungracefully
				log.Errorf("spectredCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(spectredCmd, "spectredCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	spectreminerCmd, err := common.StartCmd("MINER",
		"spectreminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("spectre-miner-Cmd.Wait", func() {
		err := spectreminerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(spectreminerCmd, "spectreminerCmd")
	return nil
}
