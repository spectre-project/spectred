...
func toRPCPayload(message appmessage.Message) (isSpectredMessage_Payload, error) {
	...
	case *appmessage.GetFeeEstimateRequestMessage:
		payload := new(SpectredMessage_GetFeeEstimateRequest)
		err := payload.fromAppMessage(message)
		if err != nil {
			return nil, err
		}
	...
	case *appmessage.SubmitTransactionReplacementRequestMessage:
		payload := new(SpectredMessage_SubmitTransactionReplacementRequest)
		err := payload.fromAppMessage(message)
		if err != nil {
			return nil, err
		}
	...
	case *appmessage.SubmitTransactionReplacementResponseMessage:
		payload := new(SpectredMessage_SubmitTransactionReplacementResponse)
		err := payload.fromAppMessage(message)
		if err != nil {
			return nil, err
		}
