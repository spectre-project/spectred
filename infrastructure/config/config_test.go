...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-spectred.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "spectred")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of spectred binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-spectred.conf")
