...
supported spectre messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Spectre Message Overview

The spectre protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which spectre network it is a part of, its type, how big it is, and a checksum
to verify validity. All encoding and decoding of message headers is handled by
this package.

To accomplish this, there is a generic interface for spectre messages named
...
of the currently supported spectre messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using spectre encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the spectre messages are intended
...
and write spectre messages. The following sections provide a quick overview of
...
The spectre network is a magic number which is used to identify the start of a
message and which spectre network the message applies to. This package provides
...
As discussed in the spectre message overview section, this package reads
and writes spectre messages using a generic interface named Message. In
...
In order to unmarshall spectre messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a spectre peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, spectreNet)
	if err != nil {
...
In order to marshall spectre messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a spectre peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, spectreNet)
	if err != nil {
		// Log and handle the error
	}
