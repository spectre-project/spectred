...
// Outpoint defines a spectre data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new spectre transaction outpoint point with the
...
// TxIn defines a spectre transaction input.
type TxIn struct {
...
// NewTxIn returns a new spectre transaction input with the provided
...
// TxOut defines a spectre transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new spectre transaction output with the provided
...
// MsgTx implements the Message interface and represents a spectre tx message.
