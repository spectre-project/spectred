...
import (
	...
	"github.com/spectre-project/spectred/infrastructure/os/execenv"
	"github.com/spectre-project/spectred/infrastructure/os/limits"
	"github.com/spectre-project/spectred/infrastructure/os/signal"
	"github.com/spectre-project/spectred/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "spectredsvc",
	DisplayName: "Spectred Service",
	Description: "Downloads and stays synchronized with the Spectre blockDAG and " +
		"provides DAG services to applications.",
}

type spectredApp struct {
	cfg *config.Config
}

// StartApp starts the spectred app, and blocks until it finishes running
func StartApp() error {
...
func (app *spectredApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start spectred: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down spectred...")
		...
		log.Infof("Spectred shutdown complete")
	}()
