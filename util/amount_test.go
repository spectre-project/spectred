...
func TestAmountCreation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			amount:   100,
			valid:    true,
			expected: 100 * constants.SompiPerSpectre,
		},
		{
			name:     "fraction",
			amount:   0.01234567,
		...
		},
		{
			...
			valid:    true,
			expected: 55 * constants.SompiPerSpectre,
		},
		{
			name:     "rounding down",
...
func TestAmountUnitConversions(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Verify that Amount.ToSPR works as advertised.
		f1 := test.amount.ToUnit(AmountSPR)
		f2 := test.amount.ToSPR()
		if f1 != f2 {
			t.Errorf("%v: ToSPR does not match ToUnit(AmountSPR): %v != %v", test.name, f1, f2)
		}

		// Verify that Amount.String works as advertised.
		s1 := test.amount.Format(AmountSPR)
		s2 := test.amount.String()
		if s1 != s2 {
			t.Errorf("%v: String does not match Format(AmountSPR): %v != %v", test.name, s1, s2)
		}
	}
}
...
func TestAmountMulF64(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Multiply 0.1 SPR by 2",
			amt:  100e5, // 0.1 SPR
			mul:  2,
			res:  200e5, // 0.2 SPR
		},
		{
			name: "Multiply 0.2 SPR by 0.02",
			amt:  200e5, // 0.2 SPR
			mul:  1.02,
			res:  204e5, // 0.204 SPR
		},
