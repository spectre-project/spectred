...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libspectrewallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libspectrewallet.UTXO{
	...
	mergeTransactionBytes, err := libspectrewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by spectrewallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libspectrewallet.UTXO, 0, endIndex-startIndex)
	totalSompi := uint64(0)
	...
	unsignedTransactionBytes, err := libspectrewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) estimateMassAfterSignatures(transaction *serialization.PartiallySignedTransaction) (uint64, error) {
	...
	transactionWithSignatures, err := libspectrewallet.ExtractTransactionDeserialized(transaction, s.keysFile.ECDSA)
	if err != nil {
		return 0, err
	}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libspectrewallet.UTXO, requiredAmount uint64) (
	additionalUTXOs []*libspectrewallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libspectrewallet.UTXO{
