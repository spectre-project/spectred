...
func (s *server) mergeTransaction(
	...
	return libspectrewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func createTransactionWithJunkFieldsForMassCalculation(transaction *serialization.PartiallySignedTransaction, ecdsa bool, minimumSignatures uint32, txMassCalculator *txmass.Calculator) (*externalapi.DomainTransaction, error) {
	...
	return libspectrewallet.ExtractTransactionDeserialized(transaction, ecdsa)
}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libspectrewallet.UTXO, requiredAmount uint64, feeRate float64) (
