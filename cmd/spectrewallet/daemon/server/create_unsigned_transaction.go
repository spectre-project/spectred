...
// By having at least 10SPR in the change output we make sure that the storage mass charged for change is
// at most 1000 gram. Generally, if the payment is above 10SPR as well, the resulting storage mass will be
// in the order of magnitude of compute mass and wil not incur additional charges.
// Additionally, every transaction with send value > ~0.1 SPR should succeed (at most ~99K storage mass for payment
// output, thus overall lower than standard mass upper bound which is 100K gram)
const minChangeTarget = constants.SompiPerSpectre * 10
...
func (s *server) calculateFeeLimits(requestFeePolicy *pb.FeePolicy) (feeRate float64, maxFee uint64, err error) {
	...
	case nil:
		...
		feeRate = estimate.Estimate.NormalBuckets[0].Feerate
		// Default to a bound of max 1 SPR as fee
		maxFee = constants.SompiPerSpectre
	}
...
func (s *server) estimateFee(selectedUTXOs []*libspectrewallet.UTXO, feeRate float64, maxFee uint64, recipientValue uint64) (uint64, error) {
	...
	var mockPayments []*libspectrewallet.Payment
	if totalValue > recipientValue {
		mockPayments = []*libspectrewallet.Payment{
			{
				Address: fakeAddr,
	...
	mockTx, err := libspectrewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
...
func (s *server) estimateFeePerInput(feeRate float64) (uint64, error) {
	mockUTXO := &libspectrewallet.UTXO{
	...
	mockTx, err := libspectrewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, []*libspectrewallet.UTXO{mockUTXO})
	if err != nil {
		return 0, err
	}
	...
	mockTxWithoutUTXO, err := libspectrewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, nil)
	if err != nil {
		return 0, err
