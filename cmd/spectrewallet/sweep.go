...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libspectrewallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libspectrewallet.SpectrewalletdUTXOsTolibspectrewalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatSpr(splitTransactions[i].Outputs[0].Value), " SPR")
		totalExtracted = totalExtracted + splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatSpr(totalExtracted), " SPR")
