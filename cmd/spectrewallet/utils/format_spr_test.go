...
// Takes in a string representation of the Spr value to convert to Sompi
func TestSprToSompi(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := SprToSompi(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := SprToSompi(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateSPRAmountFormat(testCase)
