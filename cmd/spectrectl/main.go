...
import (
	...
	"github.com/spectre-project/spectred/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		spectredMessage, err := client.Post(&protowire.SpectredMessage{Payload: &protowire.SpectredMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := spectredMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	spectredMessage := &protowire.SpectredMessage{}
	err := protojson.Unmarshal([]byte(response), spectredMessage)
	if err != nil {
	...
	return marshalOptions.Format(spectredMessage)
}
